...
\end{itemize}
The web interface was later refined with a cleaner layout, model-selection controls, a side-by-side preview panel, and live run details so that a user can dehaze uploaded images interactively during a classroom demonstration.
...
\begin{itemize}
    ...
    \item a before-and-after visual panel that helps users inspect the dehazing effect immediately.
\end{itemize}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{../assets/web_ui_interface.png}
    \caption{Live Gradio-based interactive dehazing web application. Users upload a hazy image, select a model variant (baseline, residual, or skip-connection), and receive real-time dehazed output with before-and-after comparison view.}
    \label{fig:webmockup}
\end{figure*}

\section{Conclusion and Future Scope}
\section{Conclusion and Future Scope}
This paper presented a modular academic implementation of image dehazing using AOD-Net and two lightweight extensions: residual learning and skip connections. The project reproduced the baseline AOD-Net workflow, organized it into a reusable notebook and web application, and evaluated all three variants on the complete RESIDE-6K dataset (7000 image pairs). Under the current setup, the skip-connection model achieved the highest validation PSNR (20.20 dB), while the baseline remained highly competitive (17.88 dB) and the residual model showed competitive performance (16.88 dB). All models were successfully trained on the full dataset, demonstrating scalability and robustness of the implementation.

Future work will focus on: (1) adding SSIM and perceptual loss metrics for stronger evaluation beyond PSNR, (2) designing an improved residual branch with deeper supervision and multi-scale features, (3) extending the web application with interactive before-and-after slider tools and batch processing capabilities, (4) exploring attention mechanisms and transformer-based variants for comparison, and (5) deploying the application as a cloud-based service for classroom and research use.
