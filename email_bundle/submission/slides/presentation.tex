...
\begin{frame}{Conclusion}
  \begin{itemize}
    \item Successfully implemented and trained three AOD-Net variants (baseline, residual, skip) on full RESIDE-6K dataset (7000 image pairs).
    \item Skip-connection model achieved best PSNR (20.20 dB) compared to baseline (17.88 dB) and residual (16.88 dB) on full dataset.
    \item Built a complete academic pipeline: notebook $\rightarrow$ trained models $\rightarrow$ interactive web demo.
    \item Delivered reproducible code, detailed documentation, and presentation-ready assets.
    \item Demonstrated practical application with Gradio-based web interface for real-time image dehazing.
  \end{itemize}
  
  \vspace{0.8em}
  {\centering \textbf{Project artifacts:} Notebook, checkpoints, web app, slides, and IEEE-style paper. \par}
\end{frame}

\begin{frame}[fragile]{Interactive Web Application}
  \centering
  \includegraphics[width=0.95\linewidth,height=0.82\textheight,keepaspectratio]{../assets/web_ui_interface.png}
  
  \vspace{0.2em}
  {\small Live Gradio-based interactive dehazing interface: upload image $\rightarrow$ select model $\rightarrow$ view dehazed output \& comparison.}
\end{frame}
