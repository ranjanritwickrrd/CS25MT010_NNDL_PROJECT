...
This paper presented a modular and reproducible academic implementation of image dehazing using AOD-Net along with two lightweight architectural enhancements: residual learning and skip connections. The project successfully reconstructed the baseline AOD-Net pipeline from an existing implementation and extended it into a structured notebook-driven workflow and an interactive web application suitable for both experimentation and classroom demonstration.

All three model variants were trained and evaluated on the RESIDE-6K dataset, comprising approximately 7000 paired hazy and clean images. Under the current training configuration, the skip-connection model achieved the highest validation PSNR of \textbf{20.20 dB}, demonstrating its effectiveness in preserving structural details and improving perceptual quality. The baseline AOD-Net remained competitive with a PSNR of \textbf{17.88 dB}, highlighting the efficiency of the original lightweight design, while the residual variant achieved \textbf{16.88 dB}, indicating stable performance with potential for further optimization.

Overall, the results confirm that simple architectural modifications can enhance the performance of compact dehazing networks without significantly increasing model complexity. In addition to quantitative evaluation, the integration of a web-based interface further strengthens the practical usability of the system by enabling real-time image dehazing and model comparison.

Future work will focus on extending the evaluation and model design in several directions. First, incorporating additional perceptual metrics such as SSIM and perceptual loss functions will provide a more comprehensive assessment of image quality beyond PSNR. Second, the residual branch can be improved through deeper supervision and multi-scale feature extraction to better capture complex haze patterns. Third, the web application can be enhanced with interactive comparison tools, including before-and-after sliders and batch processing capabilities. Finally, more advanced architectures such as attention-based models and transformer-based approaches can be explored for comparative analysis, along with potential deployment of the system as a scalable cloud-based service for academic and research use.
